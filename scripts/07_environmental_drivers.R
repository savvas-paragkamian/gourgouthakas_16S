...
run_dbrda <- function(env_vars, label) {
  env_df <- env_raw[stats::complete.cases(env_raw[, env_vars]), ]
  samples_env <- env_df$sample_id
  bray_env <- stats::as.dist(as.matrix(dist_bray)[samples_env, samples_env])
  env_scaled <- as.data.frame(scale(as.data.frame(env_df[, env_vars])))
  rownames(env_scaled) <- samples_env
  names(env_scaled) <- env_vars

  full_formula <- stats::reformulate(env_vars)
  dbrda_full <- vegan::capscale(stats::update(full_formula, bray_env ~ .), data = env_scaled)
  dbrda_null <- vegan::capscale(bray_env ~ 1, data = env_scaled)
  dbrda_step <- tryCatch(
...
write_result(dbrda_primary$terms |> dplyr::mutate(model = "primary"), "07_dbrda_terms")
write_result(
  tibble::tibble(model = c("primary", "sensitivity"), n = c(dbrda_primary$n, dbrda_sensitivity$n),
                  r_squared = c(dbrda_primary$r2, dbrda_sensitivity$r2),
                  adj_r_squared = c(dbrda_primary$adj_r2, dbrda_sensitivity$adj_r2)),
  "07_dbrda_summary"
)
...
mantel_results <- tibble::tibble(
  test = c("community ~ depth", "community ~ depth | temperature"),
  statistic_r = c(mantel_depth$statistic, if (!is.null(mantel_depth_partial)) mantel_depth_partial$statistic else NA),
  p_value = c(mantel_depth$signif, if (!is.null(mantel_depth_partial)) mantel_depth_partial$signif else NA)
)
